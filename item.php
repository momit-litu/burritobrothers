                        ...
                        <div class="modal-body" style="padding-left: 0px; padding-right: 0px;  min-height:200px">
                            ...
                            <div id="order-div">
                                <div class="title text-center">
                                    <h4 class="text-coffee left">Your Items Have Been Added To Cart<span id="ord_title_vw"></span></h4>
                                </div>
                                <div class="buttons_wrapper" style="padding-bottom: 15px">
                                    ...
                                    if(isset($_SESSION['group_master'])){?>
                                        <div class="col-md-6 col-sm-6 col-xs-6 text-right" style="">
                                            <a href="categories.php" style="color: white"><button type="button" class="btn-main btn-small btn-primary" style="border-radius: 4px">Select More Items</button></a>
                                        </div>
                                        <div class="col-md-6 col-sm-6 col-xs-6 " >
                                            <a href="cart.php" style="color: white"><button type="button" class="btn-main btn-small btn-primary" style="border-radius: 4px" >Proceed to Cart</button></a>
                                        </div>
                                    <?php }
                                    else{ ?>
                                        <div class="col-md-4 col-sm-4 col-xs-4 text-right" style="">
                                            <a href="categories.php" style="color: white"><button type="button" class="btn-main btn-small btn-primary" style="border-radius: 4px">Select More Items</button></a>
                                        </div>
                                        <div class="col-md-4 col-sm-4 col-xs-4" style="">
                                            <a href="cart.php" style="color: white"><button type="button" class="btn-main btn-small btn-primary" style="border-radius: 4px">View Cart</button></a>
                                        </div>

                                        <div class="col-md-4 col-sm-4 col-xs-4 " >
                                            <a href="checkout.php" style="color: white"><button type="button" class="btn-main btn-small btn-primary" style="border-radius: 4px" >Checkout</button></a>
                                        </div>
                    ...
                    <div class="modal-content">
                        <div class="modal-body" style="padding-left: 0px; padding-right: 0px; height: auto !important; min-height:400px">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12 text-center">
                                <div id="select_side_ingredinet" class="text-center" style="display:none"></div>
...
<script>
    ...
    var quantity={};
    var item_id = location.search.split('?')[1].split('=')[1];
    ...
    deletItemList = (id) =>{
        delete choosed_ingredient_number[id];
        delete selected_ingredient[id];
        chose_summary();
    ...
    chose_summary= function  chose_summary() {
        ...
        $.each(selected_ingredient, function (m, selected_ingredient_item){
            ...
            if(m != item_id){
                deleteB = '<button class="btn-xs btn-danger" onclick="deletItemList('+m+')">X</button>'
            }else {
                deleteB = ''
            }
            //console.log(m)
            html+='<div class="col-md-12 col-sm-12 col-xs-12 padding-left-0"><div class="col-md-9 col-sm-9 col-xs-9 padding-left-0"><label style="text-transform: capitalize;">'+deleteB+'<b> '+selected_ingredient_item["name"]+'</b></label></div><div class="col-md-3 col-sm-3 col-xs-3  text-right">'+currency_symbol+''+parseFloat(selected_ingredient_item["base_price"]).toFixed(2)+'</div></div>\n'
                    ...
                    $.each(data, function (j, ingredient) {
                        if(j!='option_name' && j!='name'){
                            html+='<div class="col-md-12 col-sm-12 col-xs-12">\n' +
                                ...
                                '    <div class="col-md-3 col-sm-3 col-xs-3  text-right">'+currency_symbol+''+parseFloat(ingredient['price']).toFixed(2)+'</div>\n' +
                                '  </div>\n';
            ...
            html+=' <div class="col-md-12 col-sm-12 col-xs-12 checkout-total" style="align-content: baseline; margin-top: 0px; padding: 0px"></div>\n' +
                ...
                '       <div class="col-md-3 col-sm-3 col-xs-3 text-right"><b>'+currency_symbol+''+parseFloat(total_price).toFixed(2)+'</b></div>\n' +
                '  </div>\n'+
                '    <div class="col-md-12 col-sm-12 col-xs-12 " >\n' +
                ...
                '       <div class="col-md-3 col-sm-3 col-xs-3 text-right"><b>'+currency_symbol+''+(total_price*quantity[m]).toFixed(2)+'</b></div>\n' +
                '  </div>'
        ...
        if(selected_ingredient[this_item_id][option_id]){
            ...
            if(selected_ingredient[this_item_id][option_id][ing_id]){
                ...
                if(item_choice_limit[this_item_id][option_id]['maximum']>choosed_ingredient_number[this_item_id][option_id] || item_choice_limit[this_item_id][option_id]['maximum']==0){
                    ...
                    return 2;
                }else if(item_choice_limit[this_item_id][option_id]['maximum']==1 && choosed_ingredient_number[this_item_id][option_id]==1){
                    selected_ingredient[this_item_id][option_id] = {}
                    selected_ingredient[this_item_id][option_id]={'option_name':option_name}
                    selected_ingredient[this_item_id][option_id][ing_id]={'name':ing_name, 'price':ing_rate}
                    chose_summary()
    ...
    set_item_header = function set_item_header(data) {
        base_price=parseFloat(data['price']).toFixed(2);
        ...
        base_price_view = data['price']>0 ? '('+currency_symbol+''+parseFloat(data['price']).toFixed(2)+')':''
        ...
        var html='<div class="row"><div  class="col-md-8 col-sm-6 col-xs-7"><h4 style="text-transform: uppercase;" id="item_name">'+data['name']+' '+base_price_view+'</h4></div>' +
    ...
    set_ingredient = function set_ingredient(data, category_id, item_id, item_name, item_price,option_id,class_name){
        ...
        $.each(data, function(i,datas){
			...
			price = parseFloat(datas['price'])>0? currency_symbol+parseFloat(datas['price']).toFixed(2):'';
			//alert(price);
            if(ingredient_image_display=="display: none"){
                var img = ' <div class="shop-main-list" style=" border-radius: 15px">\n' +
                    '                        <div class="shop-product-ingredient" style="border-radius: 15px 15px 0px 0px; color:  ">\n' +
                    ...
                    '                            <div class="cart-overlay-wrap hide '+class_add+'" style="background-color: #372727; opacity: .5; max-width: 120px" >\n' +
                ...
                var img ='           <div class="shop-main-list" style=" border-radius: 15px">\n' +
                    '                        <div class="shop-product-ingredient" style="border-radius: 15px 15px 0px 0px; margin-bottom: 0px; padding-bottom: 0px">\n' +
                    '                            <img src="'+project_url+'admin/images/'+photo+'" alt="" style="border-radius: 17px 17px 17px 17px; max-height: 70px; max-width: 70px">\n' +
                    '                            <div class="cart-overlay-wrap hide '+class_add+'" style="background-color: #372727; opacity: .5" >\n' +
                    '                                <div class="cart-overlay" >\n' +
                    '                                    <i class="fa fa-check" style="font-size:48px;color: white; opacity: 1"></i>\n\n' +
                    '                                </div>\n' +
                    '                            </div>\n' +
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $.each(data['side_item'], function (i,datas) {
                    ...
                    html_side+='<div class="col-md-12 col-sm-12 col-xs-12" id="additional_items_'+i+'" style="background-color: white; padding-top: 25px; padding-bottom: 20px; margin-bottom: 15px">\n' +
                    ...
                    $.each(datas, function (j, items) {
						price = parseFloat(items['price'])>0? currency_symbol+ parseFloat(items['price']).toFixed(2):'';
                        var html='<div class="col-md-12 col-sm-12 col-xs-12 isotope-item breakfast">\n' +
                            '         <div class="menu-list" style="padding-left: 0px">\n' +
                            '             <h6 class="text-capitalize margin-bottom-0 -align-left" style="margin: 0px !important;">'+ items['name']+' <span>'+ price+'</span></h6>' +
                            '          </div>\n' +
                            '        </div>\n'
                        ...
                        html_side+= '<div class="additional_items col-md-6 col-sm-6 col-xs-12 margin-bottom-10" style=" overflow: hidden; border-width: 1px; cursor: pointer">\n' +
                            ...
                            '       <input type="hidden" class="item_rate"  value="'+price+'" />\n' +
                ...
                $('#option_body').html($('#option_body').html()+html_side);
    ...
    $('.additional_items').on('click', function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                html = ''
				price = parseFloat(data['item']['price'])>0? currency_symbol+ parseFloat(data['item']['price']).toFixed(2):'';
                html22 = '<div class="row"><div  class="col-md-8 col-sm-6 col-xs-7"><h4 style="text-transform: uppercase;" id="item_name">'+data['item']['name']+price+'</b></h4></div>' +
                    '     <div class="price-textbox price-textbox-item col-md-2 col-sm-6 col-xs-5" style="text-align: right">\n' +
                    '           <span class="minus-text" onclick="minusProd('+data['item']['item_id']+')" style="padding-left: 20px;font-size: 30px;top: 5px !important;">-</span>\n' +
                    '           <input name="quantity[]" id="item_'+data['item']['item_id']+'" placeholder="1" type="text" value="1">\n' +
                    '           <span class="plus-text" onclick="addProd('+data['item']['item_id']+')" style="padding-right: 10px;font-size: 30px;top: 5px !important;">+</span>\n' +
                    '     </div></div>\n'
				...
				$('.shop-main-list').hover(
				  function() {
					   if(!$(this).find('.cart-overlay-wrap').hasClass('selected_ingredient')){
						$(this).find('.cart-overlay-wrap').removeClass('hide');
					  }
				  }, function() {
					  if(!$(this).find('.cart-overlay-wrap').hasClass('selected_ingredient')){
						$(this).find('.cart-overlay-wrap').addClass('hide'); 
					  }
				  }
				);	
	...
	$('.options-ingredient').on('click', function(){
        ...
        var this_item_name = $(this).find('input:eq(5)').val();
        var this_item_price = $(this).find('input:eq(6)').val();
        var  add_to_meal_return = add_to_meal(option_id,option_name,ing_id,ing_rate,ing_name,cat_id, this_item_id, this_item_name,this_item_price)
    ...
    function confirm(){
        ...
        side_id = $('#side_item_id_hidden').val();

        //$.each(choosed_ingredient_number, function (i, choosed_ingredient_number_single_item) {
            //alert(i)
            //console.log(choosed_ingredient_number_single_item)
        select_ingredinet_check = 1;

            $.each(choosed_ingredient_number[side_id], function (j, ingredient) {

                if (parseInt(item_choice_limit[side_id][j]['minimum']) > parseInt(ingredient) || parseInt(item_choice_limit[side_id][j]['required']) > parseInt(ingredient)) {
                    success_or_error_msg('#select_side_ingredinet', 'danger', "Please Select " + item_choice_limit[side_id][j]['name'] + " for <b>"+selected_ingredient[side_id]['name'] +" </b>", "#side_order");
    ...
    addToCart = function addToCart(){
        ...
        if(Object.keys(choosed_ingredient_number).length<=1 && is_combo==0 && category_id != 6 && category_id != 45 && sideInCart==0 && cart_side_check==0){
            $('#additional_items_beverages	').focus();
            success_or_error_msg('#select_side','warning',"No BEVERAGE has been selected. You may select one Now or proceed with  “Add to Cart”","#side_order");
            cart_side_check=1
        }
