    ...
    case "insert_or_update":
        ...
        if(isset($coupon_id) && $coupon_id == ""){
            ...
            if($return){
                ...
                $sql = "SELECT ci.customer_id, ci.email, ci.full_name, c.cupon_no, c.offer_title, c.amount, c.c_type,c.start_date , c.end_date, c.id
                        FROM cupons c
                        LEFT JOIN cupon_customer_group ccg ON ccg.cupon_id = c.id 
                        LEFT JOIN customer_group_member cgm ON cgm.group_id = ccg.customer_group_id AND cgm.status = 1
                ...
                foreach ($cuponUser as $key=>$users){
                    ...
                    $temsql="SELECT id from notification WHERE notified_to = ".$users['customer_id']." AND cupon_id= ".$users['id'];
                    //echo $temsql;
                    $checkNotification = $dbClass->getSingleRow($temsql);
                    //var_dump($checkNotification);
                    if(!$checkNotification['id']) { $dbClass->insert("notification", $columns_value);}


                    $to 	 = $users['email'];
                    $from 	 = $dbClass->getDescription('web_admin_email');
                    $subject = 'Burrito Brothers offering a discount of '.$amount;
                    $body 	 = '<p>Dear '.$users["full_name"].',</p><p>You have been offered '. $amount.' discount on your order in OurBurritoBrothers. 
                        To get this offer use coupon code: '. $coupon_no.'. </p><p>To choose your items please visit <a href="'.$web_url["website_url"].'"> OurBurritoBrothers</a>.</p>
                        <p>Best Regards</p><p>OurBurritoBrothers</p><p>Note: Please Do not reply this email.</p>';
            ...
            if($return){
                ...
                foreach ($cuponUser as $key=>$users){
                    ...
                    //echo $users['id'];
                    $details = $users['offer_title'].": ".$amount." discount for you. Please apply coupon code (".$users['cupon_no'].") which is valid from ".$users['start_date']." to ".$users['end_date'];
                    $columns_value=array(
                        'details'=>$details,
                        'status'=>0,
                        'notification_user_type'=>0,
                    ...
                    $from 	 = $dbClass->getDescription('web_admin_email');
                    $subject = 'Burrito Brothers offering a discount of '.$amount;
                    $body 	 = '<p>Dear '.$users["full_name"].'</p>You have been offered '. $amount.' discount on your order in OurBurritoBrothers. 
                        To get this offer use coupon code: '. $coupon_no.'. To choose your items please visit <a href="'.$web_url["website_url"].'"> OurBurritoBrothers</a>.</p><p>
                        Best Regards</p><p><b>OurBurritoBrothers</b></p><p>Note: Please Do not reply this email.</p>';
