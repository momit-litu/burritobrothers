...
switch ($q){
    ...
    case "grid_data":
       ...
       // echo $countsql;die;
        ...
        $total_pages = ($limit)?$total_records/$limit:1;
    ...
    case "update_order_status":
        ...
        if($return){
            ...
            //$subject = "Your order no #$order_id has been $subject by Burrito Brothers";
            //$body 	= "Dear ".$email['full_name']."<br>Your order". $order_id." has been". $subject." by Burrito Brothers. <br>Thanks<br>Our BurritoBrothers";

            if($status_id == 2){
                $subject = "Your order no #".$order_id." has been Rejected/Canceled by Burrito Brothers";
                $body 	 = "<p>Dear ".$email['full_name']."</p><p>We  apologise but we cannot fulfill  your order ". $order_id."   at this time. </p><p>You may contact the store at  ".$contactNo." for more information.</p><p>Thank You,</p><p>OurBurritoBrothers</p>";
                $dbClass->sendMail($email['email'], $subject, $body);
            }
            else if($status_id == 3){
                $subject = $order_id." Order ".$email['full_name']." Confirmation from Burrito Brothers ";
                $body 	 = "<p>Dear ".$email['full_name']."</p><p>Your order". $order_id." has been Received by Burrito Brothers. </p><p>Thanks</p><p>OurBurritoBrothers</p>";
                $dbClass->sendMail($email['email'], $subject, $body);
                ...
                $body 	 = "<p>Dear ".$email['full_name']."</p><p>Your order ". $order_id." has been Ready for  pick. </p><p>Thanks</p><p>OurBurritoBrothers</p>";
                $dbClass->sendMail($email['email'], $subject, $body);
            }
            else if($status_id == 5){
                //$details = " Your Order (".$order_id.") is Delivered ";
                $subject = "Your order no #".$order_id." has been picked up.";
                $body 	 = "<p>Dear ".$email['full_name']."</p><p>Your order". $order_id." has been picked up </p><p>Thanks</p><p>OurBurritoBrothers</p>";
                $dbClass->sendMail($email['email'], $subject, $body);
            }
            $id = $dbClass->getSingleRow("SELECT order_id FROM order_master WHERE invoice_no='$order_id'");
            $column_array = array(
    ...
    case "get_order_details_by_invoice":
        ...
        if($group_order_id['group_order_id']==0){
            ...
            $sql = "SELECT ci.full_name,ci.customer_id, ci.email as email, go.discount_amount, ci.contact_no as mobile, gi.name,go.payment_status,
                    ...
                    case go.payment_method when 1 then 'Cash' when 3 then 'Paypal' when 2 then 'Loyalty'  else 'Square'  end payment_method,
...
//var_dump($status_id);
        ...
        if($return){
            ...
            $body 	 = "<p>Dear ".$email['full_name'].",</p><p>We apologise that we  are not able to fulfill your order ". $order_id."
                A refund of ".$id['total_paid_amount']." has been issued. </p><p>Thank you for patronage.</p><p>Sincerley.</p><p>OurBurritoBrothers</p>";
            $dbClass->sendMail($email['email'], $subject, $body);
