...
//$orderConfirm->afterPayment('BB082004861');
//$orderConfirm->afterPayment('BB082004737');
...
if (!isset($_POST["txn_id"]) && !isset($_POST["txn_type"])) {
    ...
    else if (strpos($item_name, 'BBG') !== false) {
        $paypalConfig = [
            'email' => $enableSandbox ? $paypal_sandbox : $paypal_email,
            'return_url' => $next_url,
            'cancel_url' => $project_url.'account',
    ...
    //$orderConfirm->afterPayment('BB082004862');
    ...
    //$orderConfirm->afterPayment('BB082004864');

    if (checkTxnid($data['txn_id'])) {
        $item_number= trim(preg_replace('/\s+/', '', $data['item_number']));
        //$orderConfirm->afterPayment('BB082004864');z
        try {
