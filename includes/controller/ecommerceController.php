...
switch ($q){
    ...
    case "addToCart":
        ...
        if(isset($ingredient)){
            foreach ($ingredient as $id=>$single_ingredient){
                $category =$dbClass->getSingleRow("SELECT category_id, c.photo, c.name as category_name from items i left join category c ON c.id=i.category_id WHERE  item_id=$id");
                $tem_cart[$id.'_'.$single_ingredient['price']]['item_name']=$single_ingredient['name'];
                $tem_cart[$id.'_'.$single_ingredient['price']]['cart_key']=$id.'_'.$single_ingredient['price'].rand();
                ...
                $tem_cart[$id.'_'.$single_ingredient['price']]['category_id']=$category['category_id'];
                //special case: if no item imagethen save the category image
				$tem_cart[$id.'_'.$single_ingredient['price']]['category_name']=$category['category_name'];
				$tem_cart[$id.'_'.$single_ingredient['price']]['quantity']=$quantity[$id];
                $tem_cart[$id.'_'.$single_ingredient['price']]['item_image']=($category['photo']!='')?$category['photo']:'images/category/nofoodimage.png';
    ...
    case "checkout":
        ...
        if(!isset($_SESSION['cart']) || empty($_SESSION['cart']) || (empty($_SESSION['customer_id']) && empty($_SESSION['group_master'])) || ($_SESSION['customer_id']=="" && empty($_SESSION['group_master'] ))){echo "0"; die;}
        else 	$cart = $_SESSION['cart'];
    ...
    case "repeat_order":
        ...
        $sql2 = "SELECT m.*, c.photo, c.name as category_name, i.category_id FROM order_details m 
				LEFT JOIN items i ON i.item_id=m.item_id
				left join category c ON c.id=i.category_id WHERE m.order_id=".$order['order_id'];
        ...
        foreach ($result as $row) {
            ...
            $cart_key = $row['item_id'].'_'.$row['item_rate'].rand();
            $tem_cart[$cart_key]['item_name']=$item_name['name'];
			$tem_cart[$cart_key]['category_name']=$row['category_name'];
            ...
            $tem_cart[$cart_key]['item_image']=$row['photo'];
