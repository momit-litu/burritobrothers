...
class orderConfirm {
    ...
    function orderConfirmationEmail($order_id){
        ...
        if($customer_email){
            $sql = "SELECT m.order_id, m.customer_id,
                    ...
                    case payment_method when 1 then 'Cash' when 2 then 'loyalty point' when 3 then 'card'  else 'Square'  end payment_method,
           ...
           // $stmt = $conn->prepare($sql);
			...
			if($paid_status!='Paid' && ($pay_methode==3 || $pay_methode==4)){
				...
				$subject = "#". $invoice_no." Order Paid Confirmation from Burrito Brothers";
            ...
            $body .= "<link rel='stylesheet' type='text/css' href='".$web_url["website_url"]."plugin/bootstrap/bootstrap.css'>
                        <div id='order-div'>
                            <div class='title text-center' style='text-align: center'>
                                <img src='".$web_url["website_url"]."admin/images/banner/burritoLogo.png' alt='' style='alignment: center'>
    ...
    function paymentAdd($txnid, $payment_amount, $payment_status, $method, $itemid){
        ...
        try {
            ...
            if (strpos($item_number, 'BBG') !== false){
